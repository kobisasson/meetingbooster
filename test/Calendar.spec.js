    ...
    describe('Test basic APIs', function () {
       it('Calendar command line test', function (done) {
            ...
            shell.execAsync('node app.js')
                .then(function () {
                    shell.cd(dirname);
                    const expected = fs.readFileAsync( "./test/CalendarTest/expected/Boris-Guzman.ics", 'utf-8');
                    const actual = fs.readFileAsync(  "./test/CalendarTest/Boris-Guzman.ics", 'utf-8');
                    Promise.props({
                        expected: expected,
                        actual: actual
