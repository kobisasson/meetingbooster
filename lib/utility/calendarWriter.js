...
var  calendarWriter = function (){
    ...
    atom.beginTag =  AB().validate(tags ,C.object).for(function (name) {
    ...
    atom.endTag = AB().validate(C.object).for(function () {
        atom.endNode();
        return atom;
    });

    atom.addTag =  AB().validate(tags ,C.stringOrNil,C.objectOrNil,C.object).for(function (name, value, props) {
